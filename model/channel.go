...
type Channel struct {
	...
	PlanQuota            string  `json:"plan_quota" gorm:"type:text"`
	PlanQuotaUpdatedTime int64   `json:"plan_quota_updated_time" gorm:"bigint"`
...
func (channel *Channel) UpdatePlanQuota(planQuota string) {
	err := DB.Model(channel).Select("plan_quota_updated_time", "plan_quota").Updates(Channel{
		PlanQuotaUpdatedTime: common.GetTimestamp(),
		PlanQuota:            planQuota,
	}).Error
	if err != nil {
		common.SysLog(fmt.Sprintf("failed to update plan quota: channel_id=%d, error=%v", channel.Id, err))
	}
}
